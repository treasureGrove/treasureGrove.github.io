...
// =====================================================================================================
// NOTE: The branch_* / shared_* / ripple_* params in the Wind-Branch/Shared/Ripple groups below are
//       DECLARED and USED in the shared header "TA/speedtree/speedtree_games_wind.hlsli", NOT in this
//       file. Do NOT delete these //#param entries just because you cannot find a matching `float xxx;`
//       declaration or usage locally -- they drive the shared SpeedTree wind functions. Keep them in
//       sync with the header and the branch material so branches and leaves stay synchronized.
// =====================================================================================================
...
//#group Wind-Ripple = {index(9)}
//#param float ripple_enabled = {min(0.0), max(1.0), default(1.0), group(Wind-Ripple), index(0)};
//#param float ripple_speed = {min(0.0), max(20.0), default(0.5), group(Wind-Ripple), index(1)};
//#param float ripple_directional = {min(0.0), max(10.0), default(1.734), group(Wind-Ripple), index(2)};
//#param float ripple_planar = {min(0.0), max(10.0), default(1.734), group(Wind-Ripple), index(3)};
//#param float ripple_independence = {min(0.0), max(20.0), default(1.0), group(Wind-Ripple), index(4)};
//#param float ripple_flexibility = {min(0.0), max(10.0), default(0.0), group(Wind-Ripple), index(5)};
//#param float ripple_shimmer = {min(0.0), max(2.0), default(0.5), group(Wind-Ripple), index(6)};
//#param float ripple_intensity = {min(0.0), max(2.0), default(0.7), group(Wind-Ripple), index(7)};
//#param float scene_ripple_wind_influence = {min(0.0), max(1.0), default(0.05), group(Wind-Ripple), index(8)};

//#group Interactive = {index(10)}
...
#define FOLIAGE_ENABLE_RIPPLE   // frond is a leaf material -> enable the Ripple layer
...
float4 customWorldTransformation(float4 input_world_position, inout CommonVertexShaderAccessibleParameters parameters) {
    ...
    //-------------------------------------------------------------------------------------------------------
    // Unified SpeedTree-style wind layering: Shared (canopy) -> Branch (color G) -> Ripple (color R).
    // All layers share one wind direction / strength / time base; each preserves segment length (bend).
    //-------------------------------------------------------------------------------------------------------
    float global_wind01 = blended_wind.w;

    float4 wind_result = input_world_position;

    // Unified SpeedTree Games wind: Shared -> Branch -> Ripple (frond = leaf material, ripple ON).
    applyGamesWind(wind_result.xyz, parameters.world_normal.xyz, instance_pos, instance_scale.z, wind_direction, wind_dir_nor, global_wind01, parameters.instance_attribute.random_value, per_frame_time.x, parameters.color, true);
    ...
    // Pack instance center for PS sphere-normal reconstruction.
    parameters.custom_data0.r = instance_pos.x;
    parameters.custom_data0.b = instance_pos.y;
    parameters.custom_data0.a = instance_pos.z;
...
float4 customPreviousWorldTransformation(float4 input_previous_world_position, inout CommonVertexShaderAccessibleParameters parameters) {
    ...
    // Unified SpeedTree Games wind: Shared -> Branch -> Ripple (frond = leaf material, ripple ON).
    applyGamesWind(wind_result.xyz, parameters.world_normal.xyz, instance_pos, instance_scale.z, wind_direction, wind_dir_nor, global_wind01, parameters.instance_attribute.random_value, per_frame_previous_time.x, parameters.color, true);
    ...
    // Keep custom payload layout consistent between current/previous path.
    parameters.custom_data0.r = instance_pos.x;
    parameters.custom_data0.b = instance_pos.y;
    parameters.custom_data0.a = instance_pos.z;
...
void assembleMaterialParams(PixelParameters pixel_params, inout MaterialParameters material_params) {
    ...
    if (normal_sphere_self > 0.0) {
        float3 instance_position = float3(pixel_params.custom_data.custom_data1.r, pixel_params.custom_data.custom_data1.b, pixel_params.custom_data.custom_data1.a);
        float3 sphere_center = instance_position + instance_postion_normal_offset.xyz;
        float3 sphere_normal = SafeNormalize(pixel_params.world_position.xyz - sphere_center);
